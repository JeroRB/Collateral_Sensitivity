...
```{r}
path_to_souce_data <- "~/Documents/GitHub/Collateral_Sensitivity/SourceData/"
```
...
ATLAS<-readxl::read_xlsx(paste0(path_to_souce_data, "Atlas_Reuse_Data.xlsx"))

# Antibiotic groups
Antibiotic_fam<-read.delim(paste0(path_to_souce_data, "antibiotic_groups"))
...
Key_MICs<-read.table(paste0(path_to_souce_data,"key_MIC.txt"))
...
mics_completo <- read_excel(paste0(path_to_souce_data, "Source Data.xlsx"), sheet="MIC Data") 
...
spp_short<-read.csv(paste0(path_to_souce_data, "Spp.csv" ))
...
qPCR_data <- read_excel(paste0(path_to_souce_data, "Source Data.xlsx"), sheet="Supplementary Figure 3")

# Rearranges factors for beautiful plotting
order_of_treatment <- c("LB", "COL", "AZI", "ERT")  
order_of_strains <-c("pOXA-48","pBAD-OXA-48+Arabinose","pBAD-OXA-48","pUN-OXA-48") 
...
#Plots Supp. Figure 3
qPCR_data %>% 
  ggplot(aes(log2FC, x=treatment , color=strain, fill=strain))+
...
summary(aov(log2FC~strain, data=dataAOV))
TukeyHSD(aov(log2FC~strain, data=dataAOV))
```
